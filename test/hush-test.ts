...
describe("Hush Hush anonymity pools with fresh ERC20", () => {
    ...
    describe("Factory", () => {
        ...
        it("ossify", async () => {
            ...
            await expect(factory.setVerifiers(
                zero_address,
                zero_address,
                zero_address
            )).to.be.revertedWith("Factory: verifiers ossified");
        });

        it("genesis", async () => {
            await expect(factory.genesis(
                token.address,
                depositAmount.mul(2)
            )).to.be.revertedWith("Factory: genesis already defined");
        ...
        it("deploy additional pools", async () => {
            await expect(factory.connect(user).deployERCPool(
                token.address,
    ...
    describe("Deposits", () => {
        ...
        describe("invalid deposits - fails", () => {
            ...
            it("single deposit, too few approved tokens", async () => {
                ...
                await expect(hush.connect(user).deposit(
                    solidityProof,
                    signals
                )).to.be.revertedWith(
            ...
            it("single deposit, invalid inputs and proof", async () => {
                ...
                await expect(hush.connect(user).deposit(
                    solidityProof, sigs
                )).to.be.revertedWith("HushPool: oldroot not matching");
                sigs[1] = signals[1];
                ...
                await expect(hush.connect(user).deposit(
                    solidityProof, sigs
                )).to.be.revertedWith("HushPool: index not matching");
                sigs[2] = signals[2];
                ...
                for (let i = 0; i < 8; i++) {
                    ...
                    await expect(hush.connect(user).deposit(
                        proof, signals
                    )).to.be.revertedWith("HushPool: proof contains invalid elements");
            ...
            it("multi deposit, 7 deposits zero not last", async () => {
                for (let i = 0; i < 7; i++) {
                    ...
                    const { solidityProof, signals } = await generateMultiDepositProof(commits, tree, files["MultiDeposit"]["wasm"], files["MultiDeposit"]["zkey"], true);
            ...
            it("multi deposit, invalid inputs and proof", async () => {
                ...
                // Swap root for invalid root
                sigs[1] = signals[0];
                await expect(hush.connect(user).multiDeposit(
                    solidityProof, sigs
                )).to.be.revertedWith("HushPool: oldroot not matching");
                sigs[1] = signals[1];
    ...
    describe("Withdraws", () => {
        ...
        describe("invalid withdraws - fails", () => {
            ...
            it("withdraw, fee larger than depositamount", async () => {
                ...
                await expect(hush.connect(relayer).withdraw(
                    withdrawProof,
                    withdrawSignals
            ...
            it("withdraw doublespend", async () => {
                ...
                await expect(hush.connect(relayer).withdraw(
                    withdrawProof,
                    withdrawSignals
                )).to.be.revertedWith("HushPool: nullifier reuse");
            ...
            it("withdraw, invalid inputs and proof", async () => {
                ...
                const { solidityProof, signals } = await generateWithdrawProof(note, await receiver.getAddress(), relayerFee, tree, files["Withdraw"]["wasm"], files["Withdraw"]["zkey"]);
                ...
                sigs[1] = signals[0];
                await expect(hush.connect(user).withdraw(
                    solidityProof, sigs
                )).to.be.revertedWith("HushPool: root not known");
                sigs[1] = signals[1];
    ...
    describe("Bad verifiers", () => {
        ...
        it("pause verifier, always false", async () => {
            ...
            const { solidityProof: p2, signals: s2 } = await generateDepositProof(freshNote.commitment, tree, files["SingleDeposit"]["wasm"], files["SingleDeposit"]["zkey"]);
            await token.connect(user).approve(hush.address, depositAmount);
