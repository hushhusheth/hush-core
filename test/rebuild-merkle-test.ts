...
describe("Recreate tree from events", () => {
	...
	it("4 single deposits + 1 multi-deposit, then recreate tree", async () => {
		...
		expect(await pool.leafCount()).to.equal(12, "Pool has unexpected size");
		expect(poolTree.totalElements).to.equal(12, "Recreated tree has unexpected size");
		expect(await pool.getLastRoot()).to.equal(poolTree.root.toString(), "Roots not matching");

		// Rebuild tree
		let rebuildTree = await buildTree(pool, blockNumber - 2000, depth);
		...
		const { solidityProof: depositP, signals: depositS } = await generateDepositProof(
		...
		expect(await pool.getLastRoot()).to.equal(rebuildTree.root.toString(), "Rebuild and on-chain roots does not match");
		expect(await pool.leafCount()).to.equal(rebuildTree.totalElements, "Rebuild and on-chain leafcount does not match");
	});
});
