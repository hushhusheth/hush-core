...
describe("Hush Controller", function () {
	...
	beforeEach(async function () {
		const { deployer: _deployer } = await getNamedSigners(hre);
		deployer = _deployer;
	...
	it("renounce roles", async function () {
		await expect(controller.connect(newProposer).renounceRole(await controller.PROPOSER_ROLE(), proposer.address)).to.be.revertedWith(
	...
	it("admin revoke roles", async function () {
		await expect(controller.connect(newProposer).revokeRole(await controller.PROPOSER_ROLE(), proposer.address)).to.be.revertedWith(
	...
	it("revoke admin role, proposer + executor", async function () {
		// Create proposal
		let revokeAdminTX = await controller.populateTransaction.revokeRole(await controller.TIMELOCK_ADMIN_ROLE(), await deployer.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [revokeAdminTX.to, value, revokeAdminTX.data, predecessor, salt])
		);
	...
	it("revoke proposer role, proposer + executor", async function () {
		let revokeProposerTX = await controller.populateTransaction.revokeRole(await controller.PROPOSER_ROLE(), await proposer.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [revokeProposerTX.to, value, revokeProposerTX.data, predecessor, salt])
		);
	...
	it("revoke executor role, proposer + executor", async function () {
		let revokeExecutorTX = await controller.populateTransaction.revokeRole(await controller.EXECUTOR_ROLE(), await executor.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [revokeExecutorTX.to, value, revokeExecutorTX.data, predecessor, salt])
		);
	...
	it("grant proposer role, proposer + executor", async function () {
		let grantProposerTX = await controller.populateTransaction.grantRole(await controller.PROPOSER_ROLE(), await newProposer.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [grantProposerTX.to, value, grantProposerTX.data, predecessor, salt])
		);
	...
	it("grant executor role, proposer + executor", async function () {
		let grantExecutorTX = await controller.populateTransaction.grantRole(await controller.EXECUTOR_ROLE(), await newExecutor.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [grantExecutorTX.to, value, grantExecutorTX.data, predecessor, salt])
		);
	...
	it("grant admin role, proposer + executor", async function () {
		let grantAdminTX = await controller.populateTransaction.grantRole(await controller.TIMELOCK_ADMIN_ROLE(), await newAdmin.getAddress());
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [grantAdminTX.to, value, grantAdminTX.data, predecessor, salt])
		);
	...
	it("cancel pending propose", async () => {
		...
		let operationHash = ethers.utils.keccak256(
			abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [updateDelayTX.to, value, updateDelayTX.data, predecessor, salt])
		);
	...
	it("propose with short delay", async function () {
		...
		await expect(controller.connect(proposer).schedule(grantExecutorTX.to, value, grantExecutorTX.data, predecessor, salt, 15)).to.be.revertedWith(
	...
	it("execute function before delay", async function () {
		...
		await expect(controller.connect(executor).execute(grantExecutorTX.to, value, grantExecutorTX.data, predecessor, salt)).to.be.revertedWith(
	...
	describe("Controller with factory", function () {
		...
		it("transfer factory ownership", async function () {
			let ownershipTX = await factory.populateTransaction.transferOwnership(await deployer.getAddress());
			...
			let operationHash = ethers.utils.keccak256(
				abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [ownershipTX.to, value, ownershipTX.data, predecessor, salt])
			);
		...
		it("set verifier", async function () {
			...
			let operationHash = ethers.utils.keccak256(
				abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [setVerifierTX.to, value, setVerifierTX.data, predecessor, salt])
			);
		...
		it("update verifier TODO", async () => {
			...
			salt = toFixedHex(randomBN(31), 32);
			await controller.connect(proposer).schedule(setVerifierTX.to, value, setVerifierTX.data, predecessor, salt, 0);
			await expect(controller.connect(executor).execute(setVerifierTX.to, value, setVerifierTX.data, predecessor, salt)).to.be.revertedWith(
				"TimelockController: underlying transaction reverted"
			);

			// Verifiers initially set

			// Make a transaction to queueVerifierUpdate in HushFactory
			let queueVerifierTx = await factory.populateTransaction.queueVerifierUpdate(factory.address, factory.address, factory.address);
			salt = toFixedHex(randomBN(31), 32);
			await controller.connect(proposer).schedule(queueVerifierTx.to, value, queueVerifierTx.data, predecessor, salt, 0);

			let operationHash = ethers.utils.keccak256(
				abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [queueVerifierTx.to, value, queueVerifierTx.data, predecessor, salt])
			);
			...
			await controller.connect(executor).execute(queueVerifierTx.to, value, queueVerifierTx.data, predecessor, salt);

			// Read from the HushFactory
			...
			// Set the new verifiers
			let updateVerifierTx = await factory.populateTransaction.setVerifiers(factory.address, factory.address, factory.address);

			salt = toFixedHex(randomBN(31), 32);
			await controller.connect(proposer).schedule(updateVerifierTx.to, value, updateVerifierTx.data, predecessor, salt, 0);
			operationHash = ethers.utils.keccak256(
				abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [updateVerifierTx.to, value, updateVerifierTx.data, predecessor, salt])
			);
			...
			await expect(controller.connect(executor).execute(updateVerifierTx.to, value, updateVerifierTx.data, predecessor, salt)).to.be.revertedWith(
				"TimelockController: underlying transaction reverted"
			);

			await increaseTime(deployer, 60 * 60 * 24 * 7 + 600);

			await controller.connect(executor).execute(updateVerifierTx.to, value, updateVerifierTx.data, predecessor, salt);
		...
		it("create genesis:", async function () {
			...
			// Create the genesis
			let genesisTX = await factory.populateTransaction.genesis(token.address, 1000);
			...
			operationHash = ethers.utils.keccak256(
				abiCoder.encode(["address", "uint256", "bytes", "bytes32", "bytes32"], [genesisTX.to, value, genesisTX.data, predecessor, salt])
			);
		...
		it("create second pool", async function () {
			...
			let nextPoolTX = await factory.populateTransaction.deployERCPool(token.address, 10000);
